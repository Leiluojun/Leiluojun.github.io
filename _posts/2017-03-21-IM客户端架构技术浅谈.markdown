...
其中合并部分网络请求在IM中和常用到，这就是大家常听到的网络请求的`分包`与`合包`;但客户端频繁的像服务器发送网络请求时，为了避免网络请求回来的时间以及额外增加的包（每次请求都会带固定的包头），客户端会把多次请求合并，具体每次请求的包大小上限可根据服务器性能以及用户网络状态动态调整。同样，服务器在推送消息时也会对网络请求进行合包，此时客户端需要分包。 
```java
/**
     * 处理剩下的数据
     *
